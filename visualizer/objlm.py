...
def make_figures_allparams_onefigure(
    ...
    for instance_path in get_instance_paths(problem_name):
        ...
        for alg_path in instance_path.glob("ourragd_*.csv"):
            print(alg_path)
            ...
            ax.set_ylabel(r"$f(x_k)$, $L$, or $M_k$")
            ax.set_yscale("log")
            if legend:
            ...
            fig.savefig(f"{str(instance_path)}_objlm_maxiter{maxiter}_last{last}.pdf")
            # plt.close()
            fig.show()
...
def make_legend():
    ...
    legend_fig.show()
    fig.show()
    legend_fig.savefig("legend_objlm.pdf")
